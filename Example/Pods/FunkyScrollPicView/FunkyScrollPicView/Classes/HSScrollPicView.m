...
@interface HSScrollPicView ()<UIScrollViewDelegate>
{
...
@implementation HSScrollPicView


+ (instancetype)picViewWithLoadImageBlock: (LoadImageBlock)loadBlock {
    
    HSScrollPicView *adView = [[HSScrollPicView alloc] init];
...
- (void)jumpToLink:(UITapGestureRecognizer *)gester {
    ...
    id<HSScrollPicProtocol> adM = self.picModels[tag];
...
-(void)setPicModels:(NSArray<id <HSScrollPicProtocol>> *)picModels {
    ...
    for (int i = 0; i< count; i++) {
        ...
        id<HSScrollPicProtocol> picM = picModels[i % baseCount];
        // 1. 创建控件
